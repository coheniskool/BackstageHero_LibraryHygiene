...
- ~~`accentColor`/`coverColor` default discrepancy~~ **Resolved in Task 3, and the resolution reverses this doc's earlier guess**: the actual reference screenshots are red-cover/blue-accent, matching the DC **props schema**'s stated defaults (`accent=#3B5998`, `cover=#8C2727`), not `renderVals()`'s internal `??` fallback (`accent=#8C2727`, `cover=#6B8E23`) that Task 2 had assumed was canonical. Confirmed by a real side-by-side render vs. `screenshots/01-cover.png`/`02-most-requested.png` — see `tasks/todo-karaoke-songbook.md` Task 3.

...
Click **Library Tools** next to the folder picker for five whole-library scans, each with a **Dry run** toggle so you can preview before anything changes:
...
- **Five new library-hygiene tools** (above), reachable from the **Library Tools** button, plus the standalone `dedupe_report.py` CLI.
